...
import { Center, Loader, Stack, Group, Divider, Grid, Tooltip, Skeleton, ScrollArea } from '@mantine/core';
import { useDispatch, useSelector } from 'react-redux';
import { useLocation, useNavigate, useParams } from 'react-router-dom';
import { ContractHelper } from '../../../../helpers/ContractHelper';
import { AppDispatch, RootState } from '../../../../redux';
import { Agreement, InputField, Signer } from '../../../../types/ContractTypes';
    ...
    React.useEffect(() => {
        if (contractId) {
            contractHelper.getAgreement(contractId).then((data) => {
                ...
                setSigners(data.signers.sort((a, b) => {
                ...
                if (err.response && err.response.status === 404) {
                    ...
                    toast.error('Error loading the agreement try again later');
                }
                navigate('/agreements');
            });
        }
    ...
    return (<>
        ...
        <div>
            ...
            <Grid columns={GRID_TOTAL}>
                ...
                <Grid.Col span={GRID_SIDE}>
                    <Center>
                        <span><Button className='agreement-button' color='primary'>Signer Chat</Button></span>
                    </Center>
                </Grid.Col>
            </Grid>
                    ...
                    <Grid.Col span={GRID_SIDE}>
                        <Card style={{ height: '1080px' }}>
                            <CardBody className="p-0">
                                ...
                                {agreement && agreement.sequence && <p className='mx-3 mb-4'>
                                    Note: Signers must sign in the order of the sequence.
                                </p>}
                                <ScrollArea style={{height: 850}}>
                                    <Stack spacing='xl' className='px-3'>
                                        {signers.map((signer) => {
                                            return <SignerProgress key={signer.id} signer={signer} />
                                        })}
                                    </Stack>
                                </ScrollArea>
                            </CardBody>
                                                ...
                                                {inputElements.map(
                                                    (element, index) => {
                                                        if (element.page === pageNumber) {
                                                            return (
                                                                <SenderInputView key={element.id} inputField={element} />
                                                            );
                                                        } else {
                                                            return null;
                                                        }
                    ...
                    <Grid.Col span={GRID_SIDE}>
                        <Card style={{height: '1080px'}}>
                            <CardBody className='p-0'>
                                ...
                                {contractId && user && <SignerComments type='sender' token={contractId} userId={user.id} contractHelper={contractHelper} />}
                            </CardBody>
                        </Card>
                    </Grid.Col>
                </Grid>
