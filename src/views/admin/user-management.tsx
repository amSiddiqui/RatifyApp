...
const UserManagement: React.FC = () => {
    ...
    const authHelper = React.useMemo(() => new AuthHelper(dispatch), [dispatch]);
    const [users, setUsers] = React.useState<OrganizationUser[]>([]);

    React.useEffect(() => {
        authHelper.getOrganizationLegalEntities().then(res => {
    ...
    return (
        <>
            ...
            <Modal
                ...
                <AddUserForm onClose={() => {
                    setUserModal(false);
                }} authHelper={authHelper} legalEntities={legalEntities} businessFunctions={businessFunctions} />
            </Modal>
        </>
    );
};
